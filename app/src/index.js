...
const App = {
  ...
  lookUp: async function (){
    const { lookUptokenIdToStarInfo } = this.meta.methods;
		...
		const tokenInfo = await lookUptokenIdToStarInfo(tokenId).call()
    App.setStatus('TokenInfo: '+ tokenInfo);
  },

  transferStar: async function() {
		const { transferStar } = this.meta.methods;
		const address = document.getElementById('toAddress').value;
		const tokenId = document.getElementById('starIdToSend').value;
		await transferStar(address, tokenId).send({ from: this.account });
		App.setStatus('New Star Owner id ' + this.account);
	}
